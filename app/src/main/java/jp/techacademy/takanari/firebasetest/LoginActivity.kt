...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        createButton.setOnClickListener { v ->
            // キーボードが出てたら閉じる　InputMethodManager の hideSoftInputFromWindow メソッドを呼び出してキーボードを閉じさせる
            ...
            if (email.length != 0 && password.length >= 6 && name.length != 0) {
                // ログイン時に表示名を保存するようにフラグを立てる　mIsCreateAccountにtrueを設定します。
                // そしてcreateAccountメソッドを呼び出してアカウント作成処理を開始させる
    ...
    private fun createAccount(email: String, password: String) {
        ...
        // アカウントを作成する
        // createUserWithEmailAndPasswordメソッドでアカウント作成を行い、メソッドの引数にはメールアドレス、パスワードを与える
